...
(function () {
  ...
  this._drawPage1(doc, formData, info);
    ...
    _drawHeader(doc, systemLabel, code, fechaText, logoDataUrl) {
      ...
      if (logoDataUrl) {
        try {
          const logoW = 15; // mm (1.5cm convertido a mm)
          const logoH = 11.9; // mm (1.19cm convertido a mm)
  ...
  y = yTop + boxH + 10; // más aire antes del siguiente título
      ...
      const drawSignature = (x, baseY, title, imgDataUrl, name) => {
        ...
        // Imagen centrada con proporciones automáticas
        if (imgDataUrl) {
          try {
            const fmt = imgDataUrl.includes('png') ? 'PNG' : 'JPEG';
            
            // Función para calcular dimensiones proporcionales de la imagen
            const calculateImageDimensions = (dataUrl) => {
              const maxW = 35; // mm - ancho máximo para firmas
              const maxH = 12; // mm - alto máximo para firmas
              
              // Por defecto, usamos una proporción común para firmas (3:1)
              let aspectRatio = 3.0; // ancho:alto común en firmas
              
              // Si podemos, extraemos información del data URL
              try {
                const img = new Image();
                img.src = dataUrl;
            ...
            const { width: w, height: h } = calculateImageDimensions(imgDataUrl);
            const imgX = centerX - w / 2;
            // Posicionar la firma justo encima de la línea (línea está en baseY + 22)
            const imgY = baseY + 22 - h - 2; // 2mm de separación de la línea
            doc.addImage(imgDataUrl, fmt, imgX, imgY, w, h, undefined, 'FAST');
          } catch {}
        }
