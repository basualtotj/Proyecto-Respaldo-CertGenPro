    ...
    async _drawEvidencePages(doc, evidencias, systemLabel, code, fechaText, logoDataUrl) {
      ...
      for (const ev of evidencias) {
        try {
          ...
          // medir dimensiones reales para clasificar orientación
      ...
      for (let i = 0; i < processed.length; i += perPage) {
        ...
        const verticals = processed.filter(ev => ev.orientation === 'vertical');
        const horizontals = processed.filter(ev => ev.orientation !== 'vertical');
        let cy = gridYBase2;
