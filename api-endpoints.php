<?php
// DEBUG: Mostrar errores PHP en desarrollo
...
// Route admin API requests to admin.php
...
function createTecnico() {
    try {
        ...
        $rules = [
            'nombre' => ['required' => true, 'max_length' => 255],
            'especialidad' => ['required' => true, 'max_length' => 255],
            'email' => ['email' => true, 'max_length' => 255],
            'telefono' => ['max_length' => 20],
            'certificaciones' => ['max_length' => 255],
            'firma_digital' => ['max_length' => 255],
            'activo' => []
        ];
        $errors = Validator::validateInput($data, $rules);
        if (!empty($errors)) {
