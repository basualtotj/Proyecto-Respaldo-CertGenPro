...
try {
    switch ($action) {
        ...
        case 'table_data':
            $table = $_GET['table'] ?? '';
            if (!$table) {
                ApiResponse::error('Table parameter is required', 400);
                break;
            }
        ...
        default:
            $actions = [
                ...
                'table_data' => 'Get table data (?table=tablename)'
            ];
...
function getTableData($table) {
    ...
    $allowedTables = [
        'clientes', 'instalaciones', 'tecnicos', 'certificados', 
        'empresa', 'contadores', 'configuracion', 'checklists_templates', 
        'usuarios'
    ];
    ...
    try {
        ...
        throw new Exception("Error consultando tabla $table: " . $e->getMessage());
    }
}
